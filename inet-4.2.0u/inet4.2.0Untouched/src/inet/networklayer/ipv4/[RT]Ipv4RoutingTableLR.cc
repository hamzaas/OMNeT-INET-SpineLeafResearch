...
//  New and Improved Authors: Bryan Hill, Ethan Little
...
#include "inet/networklayer/ipv4/[RT]Ipv4RoutingTableLR.h"
...
Define_Module(Ipv4RoutingTableLR);

inline std::ostream& operator<<(std::ostream& os, const Ipv4Route& e)
{
    os << e.str();
    return os;
};

inline std::ostream& operator<<(std::ostream& os, const Ipv4MulticastRoute& e)
{
    os << e.str();
    return os;
};

Ipv4RoutingTableLR::~Ipv4RoutingTableLR()
{
...
void Ipv4RoutingTableLR::initialize(int stage)
{
...
void Ipv4RoutingTableLR::configureRouterId()
{
...
void Ipv4RoutingTableLR::refreshDisplay() const
{
    char buf[80];
    if (routerId.isUnspecified())
        sprintf(buf, "%d+%d routes", (int)routes.size(), (int)multicastRoutes.size(), (int)routingCache.size());
    else
        sprintf(buf, "routerId: %s\n%d+%d routes", routerId.str().c_str(), (int)routes.size(), (int)multicastRoutes.size(), (int)routingCache.size());
    getDisplayString().setTagArg("t", 0, buf);
}

void Ipv4RoutingTableLR::handleMessage(cMessage *msg)
{
    throw cRuntimeError("This module doesn't process messages");
}

void Ipv4RoutingTableLR::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
cModule *Ipv4RoutingTableLR::getHostModule()
{
    return findContainingNode(this);
}

void Ipv4RoutingTableLR::deleteInterfaceRoutes(const InterfaceEntry *entry)
{
...
void Ipv4RoutingTableLR::invalidateCache()
{
    routingCache.clear();
    localBroadcastAddresses.clear();
}

void Ipv4RoutingTableLR::printRoutingTable() const
{
    EV << "-- Routing table --\n";
...
void Ipv4RoutingTableLR::printMulticastRoutingTable() const
{
...
std::vector<Ipv4Address> Ipv4RoutingTableLR::gatherAddresses() const
{
...
InterfaceEntry *Ipv4RoutingTableLR::getInterfaceByAddress(const Ipv4Address& addr) const
{
...
bool Ipv4RoutingTableLR::isLocalAddress(const Ipv4Address& dest) const
{
...
bool Ipv4RoutingTableLR::isLocalBroadcastAddress(const Ipv4Address& dest) const
{
...
InterfaceEntry *Ipv4RoutingTableLR::findInterfaceByLocalBroadcastAddress(const Ipv4Address& dest) const
{
    for (int i = 0; i < ift->getNumInterfaces(); i++) {
        InterfaceEntry *ie = ift->getInterface(i);
        if (!ie->isBroadcast())
...
bool Ipv4RoutingTableLR::isLocalMulticastAddress(const Ipv4Address& dest) const
{
...
Ipv4Route *Ipv4RoutingTableLR::findBestMatchingRoute(const Ipv4Address& dest) const
{
...
//    }
    ...
    for(int j = 0; j != routingCache[dest].size(); j++)
        EV << "\n***\n***\tDest: " << dest << ", "<< routingCache[dest].at(j) << "\n***";
    //-------------------

    //Uniform random Distrubition
    // Does not work becase the uniform random distrubition needs the upper and lower bounds or the range,
    // however the routingCache[dest].size() changes
//    std::default_random_engine generator;
//    std::uniform_int_distribution<int> distribution(0, routingCache[dest].size());
//
//    int randomNumb = distribution(generator);

    //Random Routing
    int randomNumb = 0;
    if(routingCache[dest].size() > 1){
        randomNumb = rand() % (routingCache[dest].size());
    }
    EV << "\n\n\n RoutingCacheSize: " << routingCache[dest].size() << "\n\n\n";
    return (routingCache[dest]).at(randomNumb);
}

InterfaceEntry *Ipv4RoutingTableLR::getInterfaceForDestAddr(const Ipv4Address& dest) const
{
...
Ipv4Address Ipv4RoutingTableLR::getGatewayForDestAddr(const Ipv4Address& dest) const
{
...
const Ipv4MulticastRoute *Ipv4RoutingTableLR::findBestMatchingMulticastRoute(const Ipv4Address& origin, const Ipv4Address& group) const
{
...
Ipv4Route *Ipv4RoutingTableLR::getRoute(int k) const
{
...
Ipv4Route *Ipv4RoutingTableLR::getDefaultRoute() const
{
...
bool Ipv4RoutingTableLR::routeLessThan(const Ipv4Route *a, const Ipv4Route *b) const
{
...
void Ipv4RoutingTableLR::setRouterId(Ipv4Address a)
{
    routerId = a;
}

void Ipv4RoutingTableLR::internalAddRoute(Ipv4Route *entry)
{
...
void Ipv4RoutingTableLR::addRoute(Ipv4Route *entry)
{
...
Ipv4Route *Ipv4RoutingTableLR::internalRemoveRoute(Ipv4Route *entry)
{
...
Ipv4Route *Ipv4RoutingTableLR::removeRoute(Ipv4Route *entry)
{
...
bool Ipv4RoutingTableLR::deleteRoute(Ipv4Route *entry)    //TODO this is almost duplicate of removeRoute()
{
...
bool Ipv4RoutingTableLR::multicastRouteLessThan(const Ipv4MulticastRoute *a, const Ipv4MulticastRoute *b)
{
...
void Ipv4RoutingTableLR::internalAddMulticastRoute(Ipv4MulticastRoute *entry)
{
...
void Ipv4RoutingTableLR::addMulticastRoute(Ipv4MulticastRoute *entry)
{
...
Ipv4MulticastRoute *Ipv4RoutingTableLR::internalRemoveMulticastRoute(Ipv4MulticastRoute *entry)
{
...
Ipv4MulticastRoute *Ipv4RoutingTableLR::removeMulticastRoute(Ipv4MulticastRoute *entry)
{
...
bool Ipv4RoutingTableLR::deleteMulticastRoute(Ipv4MulticastRoute *entry)
{
...
void Ipv4RoutingTableLR::routeChanged(Ipv4Route *entry, int fieldCode)
{
...
void Ipv4RoutingTableLR::multicastRouteChanged(Ipv4MulticastRoute *entry, int fieldCode)
{
...
void Ipv4RoutingTableLR::updateNetmaskRoutes()
{
...
bool Ipv4RoutingTableLR::handleOperationStage(LifecycleOperation *operation, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
Ipv4Route *Ipv4RoutingTableLR::createNewRoute()
{
